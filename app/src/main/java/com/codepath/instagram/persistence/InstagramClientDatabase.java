...
public class InstagramClientDatabase extends SQLiteOpenHelper {
    ...
    public void emptyAllTables() {
        // Since this homework exercise primary focus on only posts and users, the implementation
        // just focus on that as well.
    ...
    public void addInstagramPosts(List<InstagramPost> posts) {
        ...
        try {
            ...
            for(int i=0; i<posts.size(); i++) {
                ...
                Log.i(">>>>>>>>> ", i + "=" + String.valueOf(userId));
                long imageId = addImage(post.image);

                ContentValues values = new ContentValues();
//                values.put(KEY_POST_ID, tbd);
                ...
                for (int j=0; j<2; j++) {
                    Long commentId = addComment(post.comments.get(j), postId);
                };
